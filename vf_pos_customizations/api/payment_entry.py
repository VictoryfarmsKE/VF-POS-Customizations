...
from erpnext.accounts.utils import get_account_currency, QueryPaymentLedger, get_outstanding_invoices as _get_outstanding_invoices
from erpnext.accounts.doctype.journal_entry.journal_entry import get_default_bank_cash_account
from erpnext.setup.utils import get_exchange_rate
from erpnext.accounts.doctype.bank_account.bank_account import get_party_bank_account
from vf_pos_customizations.vf_pos_customizations.api.mpesa import submit_mpesa_payment
...
def create_payment_entry(
    ...
    """
    Create a Payment Entry document for a customer payment.
    """
...
def get_bank_cash_account(company: str, mode_of_payment: str, bank_account: Optional[str] = None) -> Any:
    """
    Get the default bank or cash account for the given mode of payment.
    """
...
def set_paid_amount_and_received_amount(
    ...
    """
    Calculate paid and received amounts for the payment entry.
    """
...
@frappe.whitelist()
def get_outstanding_invoices(company: str, currency: str, customer: Optional[str] = None, pos_profile_name: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    Get outstanding sales invoices for a customer and company.
    """
    if customer:
        ...
        invoices_list = [
            ...
            and (
                not pos_profile_name
                or frappe.get_cached_value("Sales Invoice", invoice.get("voucher_no"), "pos_profile") == pos_profile_name
            )
            and invoice.outstanding_amount > 0.5 / (10 ** precision)
        ]
...
@frappe.whitelist()
def get_unallocated_payments(customer: str, company: str, currency: str, mode_of_payment: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    Get unallocated (advance) payments for a customer.
    """
    filters = {
        "party": customer,
        "company": company,
...
@frappe.whitelist()
def process_pos_payment(payload: str) -> Dict[str, Any]:
    """
    ...
    if allow_mpesa_reconcile_payments and data.selected_mpesa_payments and data.total_selected_mpesa_payments > 0:
    ...
    if allow_make_new_payments and data.payment_methods and data.total_payment_methods > 0:
    ...
    if data.selected_invoices and data.total_selected_invoices > 0:
        if allow_reconcile_payments and data.selected_payments and data.total_selected_payments > 0:
    ...
    if new_payments_entry:
        ...
        msg += "</tbody></table>"
    if all_payments_entry and data.selected_invoices:
...
@frappe.whitelist()
def get_available_pos_profiles(company: str, currency: str) -> List[str]:
    """
    Get available POS Profiles for a company and currency.
    """
